...
var FontBase="25px "+FontFamiliaBase;
...
var cnv = function(canvas, loader) {
	...
	that.inicioBase= function() {
		...
		that.btnPlayAgain=vxBoxImagen(660, 15, 130, 38, "", that.imagenPlayAgain);
		that.inicio();
	}
	...
	that.creaJuego=function(){
		...
		oJuego.ponerPuntos=function(iBien, iMal){
			that.ponerPuntos.setPuntos(iBien, iMal);
...
*/
    ...
    that.loadImagenes= function() {
		...
		myState.logo = myState.loader.addImage("../../base/images/skyrocket 6.png");    
    ...
    that.vx_maindraw= function() {
		...
		//myState.clear();
    	if(!modoHide){
			ctx.drawImage(myState.corniza, 0, -20);
			//ctx.drawImage(myState.menuactividad, myState.width-280, -20);

			ctx.drawImage(myState.logo, myState.width-150, 51);
		...
		if(!modoHide){
			...
			ctx.textBaseline = "top"; 
			ctx.fillText(myState.subtitulo, 90, 100);
