...
var vxZona= function(x, y, w, h, fill, pintar) {
	...
	that.vx_contains = function(ctx, mx, my, pNoScale) {
		...
		//console.info("vx_contains->zona:"+that.x+ ", "+that.y+ ", "+that.w+ ", "+that.h+ ", "+that.ang);
		ctx.rect(that.x, that.y, that.w, that.h);
		//ctx.strokeStyle="red";
		//ctx.stroke();
...
var vxShape=function(arrPuntos, pNombre) {
	...
	that.vx_draw = function(ctx, pOmiteStroke) {
		ctx.save();
		ctx.beginPath();
...
var vxBox = function (pX, pY, pW, pH, pNombre) {
	...
	that._setRotacion=function(pAngle){
		that.ang=pAngle
...
var vxBoxTexto = function (pX, pY, pW, pH, pNombre, pTexto) {
	...
	that.setRotacion=function(pAngle){
		that._setRotacion(pAngle);
		that.updatePosition(that.x-that.ang_x,that.y-that.ang_y);
	...
	that.vx_draw_texto = function(ctx) {
		...
		ctx.save();
		if(that.ang!=0){
			//console.info("draw con angulo:"+that.ang);
			ctx.translate(that.ang_x, that.ang_y);
			ctx.rotate(Math.PI/180*that.ang);
		}
...
//----------------------------
var vxCargaNumeros=function(){
	var that={}
	that.imagenes=[];

	that.loadImagenes=function(loader){
		for(var i=0;i<=30;i++){
			var obj=loader.addImage("../../images/num"+i+".png");
			that.imagenes.push(obj);
		}	
	}

	that.getImagen=function(num){
		return that.imagenes[num];
	}

	return that;
}

var vxPuntos_Imagen = function () {
	var that={};
	that.puntos_correctos=0;
	that.puntos_incorrectos=0;

	that.boxCorrectos=null;
	that.boxIncorrectos=null;

	that.setPuntos= function(pCorr, pIncorr, imgCorrecto, imgIncorrecto){
		that.puntos_correctos=pCorr;
		that.puntos_incorrectos=pIncorr;
		that.boxCorrectos=vxBoxImagen(25, 648, -1, -1, "", imgCorrecto);
		that.boxIncorrectos=vxBoxImagen(34, 694, -1, -1, "", imgIncorrecto);
	...
	that.vx_draw = function(ctx) {
		...
		if(that.boxIncorrectos!=null){
			that.boxIncorrectos.zona.vx_clear(ctx);
			that.boxIncorrectos.vx_draw(ctx);
		}

		if(that.boxCorrectos!=null){
			that.boxCorrectos.zona.vx_clear(ctx);
			that.boxCorrectos.vx_draw(ctx);
		}	
	...
	var transaccionEtapaAfuera=myTransicion(30, 
		...
		function(){
			...
			if(efecto_desplaza.activo){
				//console.info(efecto_desplaza.getX()+", "+efecto_desplaza.getY()+"->"+caja.origen_x+", "+caja.origen_y);
