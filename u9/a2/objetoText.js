...
{	
	...
	that.ang=0;//algulo utilizado
	that.ang_x=0;//centro x trasladado
	that.ang_y=0;//centro y trasladado
	that.callRedibuja=function(){};

	that.usarSeparadorParaMultipleRespuesta=false;
	that.SeparadorParaMultipleRespuesta="";

	that.usaLimitaIntentos=true;
	that.LimiteIntentos=2;
	that.respuestasAlmacenadas={};

	that.configuraLimiteIntentos=function(usar, limite){
		that.usaLimitaIntentos=usar;
		that.LimiteIntentos=limite;
	}
	...
	that.setFuncRedibuja=function(pfunc){
		that.callRedibuja=pfunc;
	}
	...
	that.configura = function(){
		...
		if(that.ang!=0){
			...
			that.ang_x=that.x+that.w/2;
			that.ang_y=that.y+that.h/2;
		}	

		that.ctx.font=that.font;
		that.ctx.textBaseline=that.textBaseline;
		
		that.grupoTextBox=vxGrupoTexboxV2(that.ctx, that.font, that.textBaseline, pRenglonHeight, that.ang);
		...
		for(var i=0;i<arr.length;i++)	{
			...
			var elemento=myParrafoTextoElemento(that.ctx, resCadena, isOpcion, isOpcionCorrecta, isTextBox, that.renglon, that.font, that.textBaseline);
			elemento.configuraLimiteIntentos(that.usaLimitaIntentos, that.LimiteIntentos);
			...
			if(isOpcion){
				if(that.ctx.font.indexOf("bold ")==-1)
			...
			if(isTextBox){
				...
				if(that.ang!=0){
					elemento.box=vxBox((that.x+that.puntero)-that.ang_x, (that.y+(that.renglon*that.renglonHeight))-that.ang_y, widthCadena, that.renglonHeight, "");
					elemento.box.zona.ang=that.ang;
					//console.info("elemento.box.zona.ang:"+that.ang);
				} else {
				...
				//recarga las respuestas previamente evaluadas.... para las animaciones...ajustando tamaño
				var valorRespuesta=that.respuestasAlmacenadas["tx"+i];
				if(valorRespuesta){
					if(valorRespuesta!=""){
						var widthRespuesta=that.ctx.measureText(valorRespuesta).width+10;
						elemento.box.w=widthRespuesta;
						elemento.box.h=that.renglonHeight;
						that.grupoTextBox.heightFont=that.renglonHeight;
						widthCadena=widthRespuesta;
					}	
				}
				...
				//recarga las respuestas previamente evaluadas.... para las animaciones
				if(valorRespuesta){
					if(valorRespuesta!=""){
						cajaTexto.text=valorRespuesta;
					}
				}	


				//console.info("cajaTexto.text:("+"tx"+i+"):"+that.respuestasAlmacenadas["tx"+i]+"->"+cajaTexto.text);
			} else {
	...
	that.draw=function(){
		...
		//console.info("that.font:"+that.font+"-->that.textBaseline:"+that.textBaseline);
		that.ctx.font=that.font;
		that.ctx.textBaseline=that.textBaseline;

		if(that.ang!=0){
			that.ctx.translate(that.ang_x, that.ang_y);
	...
	that.recorreCajasBuscaValor=function(valor){
		//console.info("recorreCajasBuscaValor...0");
		var contador=0;
	...
	that.setSeparadorMultipleRespuesta=function(usar, separador){
		that.usarSeparadorParaMultipleRespuesta=usar;
		that.SeparadorParaMultipleRespuesta=separador;
	}
		...
		for(var i=0;i<that.elementos.length;i++) {
			...
			if(elemento.esTextBox && elemento.activo){//} && elemento.tipo=="dragend"){
				...
				if(cajaTexto.tipo=="dragend"){
					...
					var pointX=cajaEnArrastre.getX();//*factorScale;
					var pointY=cajaEnArrastre.getY();//*factorScale;

					if(that.ang!=0){
						pointX=pointX-that.ang_x;
						pointY=pointY-that.ang_y;
					}

					pointX*=factorScale;
					pointY*=factorScale;

					if(cajaTexto.vx_contains(that.ctx, pointX, pointY)){
						//console.info("evalua_AlSoltarArrastre->"+hmTextbox.label+"=="+cajaEnArrastre.texto);

						var EvaluaRespuesta=false;
						if(that.usarSeparadorParaMultipleRespuesta){
							//console.info("evalua_AlSoltarArrastre-usarSeparadorParaMultipleRespuesta");
							if(hmTextbox.label.indexOf(that.SeparadorParaMultipleRespuesta)>-1){
								var arr=hmTextbox.label.split(that.SeparadorParaMultipleRespuesta);
								for(var i=0;i<arr.length;i++){
									//console.info("------->"+arr[i]+"=="+cajaEnArrastre.texto);
									if(arr[i]==cajaEnArrastre.texto){
										EvaluaRespuesta=true;
										break;
									}
								}
							} else {
								EvaluaRespuesta=(hmTextbox.label==cajaEnArrastre.texto);	
							}
						} else {
							EvaluaRespuesta=(hmTextbox.label==cajaEnArrastre.texto);
						}	
						...
						if(EvaluaRespuesta)	{
							...
							if(cajaEnArrastre.texto){
								//Centramos la caja de textos de la respuesta dentro de la destino
								cajaEnArrastre.x_noRotate=cajaEnArrastre.x;
								cajaEnArrastre.y_noRotate=cajaEnArrastre.y;

								var dxn=cajaTexto.x+(cajaTexto.w-cajaEnArrastre.w)/2;
								cajaEnArrastre.updatePosition(dxn+that.ang_x, cajaTexto.y+that.ang_y);
								if(that.ang!=0 && cajaEnArrastre.ang!=that.ang)
									cajaEnArrastre.setRotacion(that.ang);
							...
							//cambio num intentos cfg
							//if(elemento.intento==1){
							if(elemento.usaLimitaIntentos && elemento.intento==(elemento.LimiteIntentos-1)){	
								//that.respuestaCorrecta=false;
						...
						if(callBack)
							callBack(that.activo, elemento.cadena, cajaTexto, elemento.activo, elemento.respuestaElementoCorrecta, that.ang_x, that.ang_y);
...
var myParrafoTextoElemento=function(ctx, pCadena, pEsOpcion, pEsCorrecta, pEsTextBox, pRenglon, pfont, ptextBaseline){
	var that={};
	that.ctx=ctx;
	that.box = null;
	that.cadena=pCadena;
	...
	that.usaLimitaIntentos=false;
	that.LimiteIntentos=-1;

	that.respuestaElementoCorrecta=false;
	that.ang=0;
	that.font=pfont;
	that.textBaseline=ptextBaseline;
	...
	that.draw=function(dx){
		...
		//console.info("that.box.aplicaAlpha:"+that.box.aplicaAlpha);
...
//---------------------------------------
var vxGrupoTexboxV2=function(ctx, font, baseline, pHeightFont, pAngle){
	var that={};
	that.ctx=ctx;
	that.lista=[];
...
var vxTextBox=function(ctx, x, y, h, w, id){
	...
	that.mapKeyPressToActualCharacter = function(isShiftKey, characterCode) {
		...
		//console.info("characterCode:"+characterCode);
		...
		if (isShiftKey) {
			if (characterCode >= 65 && characterCode <= 90) {
				//console.info("characterCode:--->1");
		  		character = String.fromCharCode(characterCode);
			} else {
				//Definir al shift cuales caracteres
				//console.info("characterCode:--->2");
		  		character = characterMap[characterCode];
			}

		} else {
			if (characterCode >= 65 && characterCode <= 90) {
				//console.info("characterCode:--->3");
			  character = String.fromCharCode(characterCode).toLowerCase();
			} else {
				//console.info("characterCode:--->4");
		  		if (characterCode === 188) {
		  			//console.info("characterCode:--->5");
		    		character = ',';
		  		} else if (characterCode === 190) {
		  			//console.info("characterCode:--->6");
		    		character = '.';
		  		} else {
		  			//console.info("characterCode:--->7");
		    		//character = String.fromCharCode(characterCode);
		    		//Definir sin shift cuales caracteres
		    		character = characterMap[characterCode];
		  		}
			}
		}
